...
public class TeamService : Singleton<TeamService>, IDisposable
{
    ...
    public void SendTeamInviteResponse(bool accept, TeamInviteRequest request)
    {
        ...
        message.Request.teamInviteRes.Errormsg = accept ? "组队成功" : "对方拒绝组队请求";
    ...
    {
        var confirm = MessageBox.Show(string.Format("{0} 邀请你加入队伍", request.FromName), "队伍请求", MessageBoxType.Confirm, "接受", "拒绝");
        confirm.OnYes = () => {
