...
public class UIGuildApplyItem : ListView.ListViewItem
{
    ...
    {
        MessageBox.Show(string.Format("要通过【{0}】的公会申请么", this.Info.Name), "审批申请", MessageBoxType.Confirm,"同意加入","取消").OnYes = () => 
        {
    ...
    {
        MessageBox.Show(string.Format("要拒绝【{0}】的公会申请么", this.Info.Name), "审批申请", MessageBoxType.Confirm,"拒绝加入", "取消").OnNo = () =>
        {
