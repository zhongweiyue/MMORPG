...
namespace GameServer.Services
{
    class FriendService:Singleton<FriendService>
    {
        ...
        //收到加好友响应
        private void OnFriendAddResponse(NetConnection<NetSession> sender, FriendAddResponse response)//sender是被添加者
        {
            Character character = sender.Session.Character;
            ...
            {
                //接受了好友请求
                var requester = SessionManager.Instance.GetSession(response.Request.FromId);//请求加好友的人
                if (requester == null)
                {
                    sender.Session.Response.friendAddRes.Result = Result.Failed;
